...
public class PartidasAction extends Action 
{
  /**
  ...
  public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException
  {
    ...
    PartidasForm finform = (PartidasForm)form;
    ...
    try {
      aCalm = bdc.listarPartidas(1);              
      request.setAttribute("PartidasLista", aCalm);
      ArrayList datos = (ArrayList) request.getAttribute("PartidasLista"); 
      if (finform.getOperacion()==1)
      {   
          PartidasDetalleForm d = new PartidasDetalleForm();
          d = (PartidasDetalleForm) datos.get(it);  
          finform.setPar_codigo(d.getCodigo());       
          finform.setPar_descripcion(d.getDescripcion());
      }
      if (finform.getOperacion()==2)
      {
          try {
             if (!finform.getBoton().equals("Cancelar")) {
                String msgsql=null;
                msgsql = bdc.insertarpartidas(finform.getPar_codigo(),finform.getPar_descripcion(),fInicio.getTxt_usu(),finform.getOpcion());
                finform.setPar_codigo("");
                finform.setPar_descripcion("");
                if (!msgsql.equals("0")) {
             ...
             try {
                ...
                error.add("error", new ActionMessage("errordb", "No se pudo iniciar Partidas"));
